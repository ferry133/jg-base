...
| 17 | Discovered LAN addresses | **Never `fail` except for one case**: the serviceSelectors on `pool-discovered` / `pool-discovered-external` no longer keeping the two apart — that is a partition breach and the external address could land on the shared pool. Everything else is `warn` or `ok`: an empty pool, a reselected address (LAN clients still holding the old one), or a pool present but unused. ⚠️ A partition it cannot observe is reported as `warn` saying **"this is not a pass, it is a missing measurement"** rather than as `ok`. |
